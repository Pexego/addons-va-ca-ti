...
class account_invoice(osv.osv):
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if inv.date_invoice:
            ...
            if inv.type == 'out_invoice':
                if (not cont.fmaxs) or (cont.fmaxs <= inv.date_invoice):
                    cont.write({'fmaxs':inv.date_invoice})
                else:
                    raise osv.except_osv(_('Error'), _('La factura que esta intentando crear tiene una fecha inferior a la ultima creada: '+str(cont.fmaxs)))    

            if inv.type == 'in_invoice':
                if (not cont.fmaxps) or (cont.fmaxps <= inv.date_invoice):
                    cont.write({'fmaxps':inv.date_invoice})
                else:
                    raise osv.except_osv(_('Error'), _('La factura que esta intentando crear tiene una fecha inferior a la ultima creada: '+str(cont.fmaxps)))    
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if 'date_invoice' in vals:
            ...
            for lin in self.browse(cr, uid, ids[0]).invoice_line:
        ...
        for line in inv.invoice_line:
                ...
                if ('state' in vals and (vals['state'] == 'cancel' or vals['state'] == 'draft')) or ('estado' in vals and (vals['estado'] == 'cancel' or vals['estado'] == 'draft')):
                        values['expedicion_id'] = False
                        values['estado'] = str(inv.state)
                        vals['val_s'] = True
                else:
                    values['estado'] = str(inv.state)
                    if not line.expedicion_id and not line.noexp and not line.expedicion_id_aux:
                    ...
                    if not line.nescala and not line.expedicion_id and not line.expedicion_id_aux and not line.noexp:
    ...
    def sobreescribir_exp(self, cr, uid, ids, context = None):
        ...
        inv = self.browse(cr, uid, ids)[0]
        for line in inv.invoice_line:
                line.write( {'expedicion_id':inv.expedicion_id.id,  'afecta': str(inv.afecta), 'nescala': str(inv.nescala)})
...
class account_invoice_line(osv.osv):
    ...
    _columns = {
                    ...
                    'ffactura':fields.char('Fecha de factura', size= 256),
