...
class lin_imp_acu(osv.osv):
    ...
    _columns = {
                ...
                'impuesto_cod':fields.many2one('account.tax.code','Impuesto'),
...
class lineas_emire_aux(osv.osv):
    ...
    def borrar(self, cr, uid, ids = False, context = None):
        ...
        objb = self.pool.get('lin.imp.acu')
                ...
                for tax in ex.tax_line:
                    ...
                    if len(tax.name.split('IVA'))>1:
                        ...
                        if len(tax_s) > 0:
                            ...
                            print 'impuesto '+str(tax.name)

                            print 'Factura '+str(ex.name)+'   '+str(ex.id)
                            print 'cod impuesto '+str(tax.base_code_id.name)

                            print 'id cod impuesto '+str(tax.base_code_id.id)
                
                            values = {      
                                        'informe':inf.id,
                                        'impuesto_cod':tax.base_code_id.id,
