...
class wizard_report(wizard.interface):
    ...
    def _check_xml(self, cr, uid, data, context):
        if data['form']['landscape']==True:
            return 'report_landscape_xml'
        else:
            return 'report_xml'

    def _check_xmlx(self, cr, uid, data, context):
        if data['form']['landscape']==True:
            return 'report_landscape_xml'
        else:
            return 'report_xml'
    ...
    states = {
        ...
        'checktype': {
            ...
            'result': {'type':'form', 'arch':period_form, 'fields':period_fields, 'state':[('end','Cancel','gtk-cancel'),('checkreport','Print','gtk-print'),('checkreport_xml','Print XLS','gtk-print')]}
        },
        'checkreport_xml': {
            'actions': [],
            'result': {'type':'choice','next_state':_check_xml}
        },
        'checkreport_xmlx': {
            'actions': [],
            'result': {'type':'choice','next_state':_check_xmlx}
        },
        'checkreport': {
            'actions': [],
